...
fetch(`/api/validateRoom/${room}`).then(res => res.text()).then(res => {
    if (res == 'true') {
...
socket.on('gameStarted', message => {
    ...
    if (!itIsYourTurn && gameSettings.currentPlayer.id == you.id) {
        itIsYourTurn = true;yourTurn.play();
    }
});
...
socket.on('gameOver', players => {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            myConfetti1.reset();myConfetti2.reset();cannon.remove();
        }, 2500);
    }, 500);
});

socket.on('gamePreview', previewHexes => {
    hexes = previewHexes.map(h => new Hex(h));
});
...
function drawPlayerBox(gs, players, hexes) {
    ...
    for (let i = 0; i < players.length; i++) {
        const player = players[i];
        if (gs.moveOrder.map(m => m.id).includes(player.id)) {
            continue;
        }
        playerBox.innerHTML += `
